...
The dataset is stored in the `2afc_trials.csv` file. Each row represents a single trial and contains:
...
- `response_description`: participant's response ("left" or "right")
- `response_time`: time taken to respond (in seconds)
- `accuracy`: 1 = correct, 0 = incorrect

You can also find metadata about the experiment in `2afc_trials.yaml` in [CSVW format](https://csvw.org).
