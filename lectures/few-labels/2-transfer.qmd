...
def compute_learning_curve(features, y_arr, n_list=(5, 10, 20, 40), n_seeds=10):
    rows = []
    for n in n_list:
        accs = []
...
print(f"Target S{target_subject}: {len(y_target_all)} trials | Source (other 9): {len(y_source)} trials")
```
...
print(f"Bandpower: {bp_s1.shape}  |  Tangent: {ri_s1.shape}")
```
...
print(f"EEGNet penultimate: {en_s1.shape}")
```


```{python}
curves = pd.concat({
    "bandpower (1990s)":       compute_learning_curve(bp_s1, y_target_all),
    "Riemannian (2010s)":      compute_learning_curve(ri_s1, y_target_all),
    "EEGNet penultimate (borrowed)": compute_learning_curve(en_s1, y_target_all),
...
scores_df = pd.Series(scores, name="accuracy").rename_axis("regime").reset_index()
scores_df
```
